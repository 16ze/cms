...
export default function SuperAdminLoginPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        // Utiliser un fetch silencieux qui ne log pas les erreurs 401/403 attendues
        const response = await fetch('/api/auth/me', {
          credentials: 'include', // Important pour inclure les cookies
        }).catch(() => {
          // En cas d'erreur réseau, retourner null (pas d'utilisateur connecté)
          return null
        })

        // Si la réponse est OK (200-299), vérifier les données
        if (response && response.ok) {
          try {
            const data = await response.json()

            if (data.success && data.user?.type === 'SUPER_ADMIN') {
              // Utilisateur déjà connecté en tant que Super Admin, rediriger
              // Mais seulement si on n'est pas déjà en train de se déconnecter
              // (vérifier le paramètre query logout)
              const isLoggingOut =
            ...
            // Erreur de parsing JSON, continuer avec le formulaire
          }
        }

        // Si la réponse est 401 ou 403, c'est normal (pas d'utilisateur connecté)
        // Ne pas logger ces erreurs - elles sont attendues sur la page de login
      } catch (error) {
        // Erreur réseau ou autre, continuer avec le formulaire de login
        // Ne pas logger - c'est normal si l'utilisateur n'est pas connecté
      } finally {
        setCheckingAuth(false)
      }
    }

    // Attendre un peu avant de vérifier pour s'assurer que le cookie est bien supprimé
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirection sécurisée vers le dashboard super-admin
      // Utiliser window.location.href pour forcer la redirection
      toast.success('Connexion réussie')
      const redirectTo =
        new URLSearchParams(window.location.search).get('redirect') || '/super-admin/dashboard'
      console.log('🔄 Redirection vers:', redirectTo)

      // Utiliser window.location.href pour forcer la redirection complète
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-white/10 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-purple-200 text-sm">Accès réservé KAIRO Digital</p>
          </div>
          ...
          {error && (
            <div className="mb-6 bg-red-500/20 border border-red-500/50 rounded-lg p-4 flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-red-100 text-sm font-medium">Erreur de connexion</p>
        ...
        {/* Footer */}
        <div className="mt-6 text-center">
          <p className="text-sm text-purple-300">© 2025 KAIRO Digital. Tous droits réservés.</p>
        </div>
      </div>
    </div>
  )
